...
function engine({ai=false,energy=false}={}){
 ...
 const capture=`  if(globalThis.__capture) return {chooseHandCard,placePickedCard,tapPickR,fieldPlacesR,setPlaces,resetField,removeFieldIdx,startGame,energyR,updateEnergy,stopEnergy,spendEnergy,refundEnergy,energyAwardRoundR,returnEnergyFlight,smartPick,denyPick,playTappedCard,pSpR,linkR,enterLink,exitLink,movePlayerCursor,playSelectedCard,pCursorR,pMoveUntilR,changeControlSide,dCursorSlotR,dSlotsR,dCurR,curBusyR,cursorToCard,cancelCursor,setFrz,setLocks,setCmd,rStopR,
